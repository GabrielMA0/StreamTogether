...
const VideoPlayer = () => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await axios.post(`${process.env.VITE_SOCKET_SERVER_URL}/video/upload`, formData, {
  ...
  const handleChangeFile = async () => {
    try {
      await axios.delete(`${process.env.VITE_SOCKET_SERVER_URL}/video`);
  ...
  useEffect(() => {
    const searchVideo = async () => {
      if (!videoUrl) {
        try {
          const response = await axios.get(`${process.env.VITE_SOCKET_SERVER_URL}/video`);
          setVideoUrl(response.data)
      ...
      {isLoading ? (
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center gap-5 w-full">
          <video ref={videoRef} onPlay={handlePlay} onPause={handlePause} onSeeked={handleSeek} controls muted className="w-[90%] sm:w-2/5">
            <source src={`${process.env.VITE_SOCKET_SERVER_URL}${videoUrl.fileUrl}`} type="video/mp4" />
